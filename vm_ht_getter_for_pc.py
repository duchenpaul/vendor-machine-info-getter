...
class cookie_expired(Exception):
	def __init__(self):
		...
		print('Cookie is invalid, using password to login...')
...
vm1751 = ht_getter(post_info_2['username'], post_info_2['password'])
vm1751.login()
machine_status_page = vm1751.webpage_get('http://ht.jj1001.com/?a=machine&m=status', mobile_headers).content.decode('utf-8')
status = check_status()
print(status)
status['Machine'] = 'vm1751'
print("vm1751: " + str(status))


vm1752 = ht_getter(post_info['username'], post_info['password'])
# print(vm1752.webpage_get(url).content.decode('utf-8'))
# vm1752.webpage_get(url)
# vm1752.login_w_pass()

vm1752.login()
machine_status_page = vm1752.webpage_get('http://ht.jj1001.com/?a=machine&m=status', mobile_headers).content.decode('utf-8')
status = check_status()
print("vm1752: " + str(status))
